...
export interface PopMotionProps {
}

export interface PopMotionState {
  path: string,
...
export default class PopMotion extends React.Component<PopMotionProps, PopMotionState> {
  constructor(props: PopMotionProps) {
    super(props);
    this.state = {
      path: PATHS.neutral,
  ...
  interpolatePaths = (type: string, index: number) => {
    const interpolator = interpolate(this.state.path, PATHS[type], { maxSegmentLength: 2 })
    tween({
      duration: 300,
      ease: easing.easeInOut,
      from: { i: 0, background: "red" },
      to: { i: 1, background: "blue" }
    })
  ...
  public render() {
    return (
      <LinearGradient style={styles.gradient} colors={['rgb(243,189,67)','rgb(203,96,32)']} >
